...
export function AppSidebar() {
  ...
  const getNavCls = (path: string) => {
    ...
    return isActive(path) ? `${baseClasses} bg-sidebar-accent text-sidebar-accent-foreground font-medium` : `${baseClasses} text-sidebar-foreground hover:bg-sidebar-accent/50 hover:text-sidebar-accent-foreground`;
  };
  return <Sidebar className={`${collapsed ? 'w-14' : 'w-64'} transition-all duration-300 bg-sidebar border-sidebar-border`} collapsible="icon" side="left">
      <SidebarContent className="bg-sidebar">
        ...
        <div className="p-4 border-b border-sidebar-border">
          <div className="flex items-center gap-3">
            ...
            {!collapsed && <div>
                <h2 className="text-lg font-bold text-sidebar-foreground">ALF Chitumba</h2>
        ...
        <SidebarGroup className="px-2">
          {!collapsed && <SidebarGroupLabel className="text-sidebar-foreground/70 text-xs font-medium px-2 py-2">
              MENU PRINCIPAL
            </SidebarGroupLabel>}
